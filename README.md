...
> Tips:  
> 推荐使用独立虚拟环境；也可以使用 `uv` 指定 Python 版本运行：
...
使用配置文件运行 ：
```bash
uv run --python 3.10 main.py -c config.ini
```
...
- 显式填写课程 ID 后若没有任何匹配，程序会报错退出，不会再静默处理全部课程。需要处理全部课程时可输入 `*`。
- 视频任务默认全局串行，避免多个视频同时上报心跳造成进度回溯；文档、答题等其他任务仍可并发。
- 检测到需要按顺序解锁的课程时，该课程会自动改为单章节执行。
- 任务重试耗尽后会汇总为失败，程序不会发送“所有课程已完成”的成功通知。

### 已关闭任务点处理配置说明

在配置文件的 `[common]` 部分，可以通过 `notopen_action` 选项配置遇到已关闭任务点时的处理方式:

- `retry` (默认): 遇到关闭的任务点时重试当前章节，最多尝试 5 次，仍未开放则将该课程标记为失败
- `ask`: 遇到关闭的任务点时询问本次是重试还是跳过
